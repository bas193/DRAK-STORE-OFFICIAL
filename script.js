...
function simpanKategori() {
    console.log("👉 simpanKategori() dipanggil!");
    const db = getDB();
    ...
    if (!nama) { alert('⚠️ Sila isi nama kategori!'); return; }

    if (editId) {
        const idx = db.kategori.findIndex(k => k.id === parseInt(editId));
        if (idx !== -1) db.kategori[idx] = { id: parseInt(editId), nama, icon, jenis };
        ...
        db.kategori.push({ id: Date.now(), nama, icon, jenis });
    ...
    console.log("✅ Kategori disimpan!", db.kategori);
}
...
function loadTabelKategori() {
    ...
    if (db.kategori.length === 0) {
        tbody.innerHTML = `<tr><td colspan="5" style="text-align:center;padding:30px;color:#8B949E">Tiada kategori</td></tr>`;
        return;
    }
...
function padamKategori(id) {
    if (!confirm('⚠️ Pasti hapus?')) return;
    const db = getDB();
...
function loadDropdownKategori() {
    ...
    console.log("✅ Dropdown kategori dimuatkan!");
}

function simpanProduk() {
    console.log("👉 simpanProduk() dipanggil!");
    const db = getDB();
    ...
    if (!kategori_id || !nama || !harga) {
        alert('⚠️ Lengkapi semua maklumat!');
        return;
    }

    if (editId) {
        const idx = db.produk.findIndex(p => p.id === parseInt(editId));
        if (idx !== -1) db.produk[idx] = { id: parseInt(editId), kategori_id, nama, harga, proses };
        ...
        db.produk.push({ id: Date.now(), kategori_id, nama, harga, proses });
    ...
    loadTabelProduk();
    console.log("✅ Produk disimpan!", db.produk);
}
...
function loadTabelProduk() {
    ...
    if (db.produk.length === 0) {
        tbody.innerHTML = `<tr><td colspan="6" style="text-align:center;padding:30px;color:#8B949E">Tiada produk</td></tr>`;
        return;
    }
...
function hantarPesanan() {
    ...
    const kat = db.kategori.find(k => k.id === PRODUK_DIPILIH.kategori_id);
    const jenis = kat ? kat.jenis : "diamond";
    let teks = `🛒 PESANAN BARU — DARK STORE\n━━━━━━━━━━━━━━━━━━━━\n📦 SENARAI BARANG:\n✅ ${PRODUK_DIPILIH.nama} — ${PRODUK_DIPILIH.harga}\n━━━━━━━━━━━━━━━━━━━━\n`;
    ...
    if (jenis === "diamond") {
        ...
        teks += `👤 Nama: ${nama}\n🎮 Player ID: ${id}\n📱 No. Telefon: ${phone}\n📍 Alamat: ${note}\n💳 Cara Bayaran: QRIS / DuitNow\n━━━━━━━━━━━━━━━━━━━━\n💰 JUMLAH: ${PRODUK_DIPILIH.harga}\n`;
    } else {
        ...
        if (!link) { alert('⚠️ Sila masukkan link!'); return; }
        teks += `👤 Nama: ${nama}\n🔗 Link: ${link}\n📱 No. Telefon: ${phone}\n💬 Catatan: ${note}\n💳 Cara Bayaran: QRIS / DuitNow\n━━━━━━━━━━━━━━━━━━━━\n💰 JUMLAH: ${PRODUK_DIPILIH.harga}\n`;
    }
    teks += `\nTerima kasih kerana membeli di\n💎 DARK STORE!`;
    window.open(`https://wa.me/${OWNER_WA}?text=${encodeURIComponent(teks)}`, '_blank');
    tutupModal();
}
...
function loadFilterKategori() {
    ...
    if (!container) return;
    container.innerHTML = `<button class="filter-btn active" data-kategori="all">Semua</button>`;
    db.kategori.forEach(k => {
        container.innerHTML += `<button class="filter-btn" data-kategori="${k.id}">${k.icon} ${k.nama}</button>`;
    });
    document.querySelectorAll('.filter-btn').forEach(btn => {
        btn.addEventListener('click', function() {
            document.querySelectorAll('.filter-btn').forEach(b => b.classList.remove('active'));
            this.classList.add('active');
            loadProdukByKategori(this.getAttribute('data-kategori'));
        });
    });
}
...
function loadProdukByKategori(kategoriId = 'all') {
    ...
    if (!container) return;
    let produkList = kategoriId === 'all' ? db.produk : db.produk.filter(p => p.kategori_id === parseInt(kategoriId));
    container.innerHTML = '';
    if (produkList.length === 0) {
        container.innerHTML = `<div class="empty-state" style="grid-column:1/-1;padding:40px"><p>Tiada produk</p></div>`;
        return;
    }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    console.log("✅ Semua fungsi sedia!");
});
